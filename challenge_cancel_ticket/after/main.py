...
from challenge_cancel_ticket.after.operations import TicketUpdate
...
async def delete_ticket(ticket_id: int, database: Session=Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=404) from exc
    except operations.EventHasStarted as exc:
        raise HTTPException(status_code=400) from exc

@app.patch("/tickets/{ticket_id}")
async def update_ticket(ticket_id: int, ticket_update: TicketUpdate, database: Session=Depends(get_db)):
    try:
