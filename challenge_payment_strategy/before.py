...
PaymentHandler = Callable[[Decimal], None]
PAYMENTHANDLERS: dict[str, PaymentHandler] = {'cc': cc_payment, 'paypal': paypal, 'apple': apple_pay}
...
def handle_payment(payment_type: str, total: Decimal):
    # payment_method: PaymentMethod | None= PAYMENTS.get(payment_type)
    if payment_type in PAYMENTHANDLERS:
        PAYMENTHANDLERS[payment_type](total)
    else:
        print(f'Unknown payment type "{payment_type}"')
...
def main() -> None:
    ...
    handle_payment(payment_type, cart.total)
